...
<img
	src="/resources/draw.svg"
	alt="draw in the background"
...
<a
	href="/"
	class="absolute top-0 left-0 w-full flex 2xl:hidden justify-center items-center p-[50px] z-10"
>
...
<div class="block 2xl:hidden burger absolute top-0 left-0 z-[70] transform scale-50">
	<button
		id="burger-container"
		class="burger-container {flagBurger ? 'burger-active' : ''}"
		on:click={() => {
...
<nav
	...
	class="w-full transform fixed 2xl:static h-screen 2xl:h-[100px] transition
     ...
     {flagBurger
		? ' opacity-100 pointer-events-auto bg-custom select-auto translate-y-0'
		: ' opacity-0 -translate-y-[100vh] select-none pointer-events-none'}"
>
	<div
